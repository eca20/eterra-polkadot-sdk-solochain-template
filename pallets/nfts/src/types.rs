...
impl BitFlag for CollectionSetting {
	...
	const ALL: Self::Bits = (1 << 5) - 1;

	fn bits(self) -> Self::Bits {
		match self {
			Self::TransferableItems => 1 << 0,
			Self::UnlockedMetadata => 1 << 1,
			Self::UnlockedAttributes => 1 << 2,
			Self::UnlockedMaxSupply => 1 << 3,
			Self::DepositRequired => 1 << 4,
		}
	}
}
...
impl BitFlag for ItemSetting {
	type Bits = u64;

	const ALL: Self::Bits = (1 << 3) - 1;
...
impl BitFlag for PalletFeature {
	type Bits = u64;

	const ALL: Self::Bits = (1 << 4) - 1;
...
impl BitFlag for CollectionRole {
	type Bits = u8;
