...
def economic_gates(block_number: int = 100, block_hash: str = BLOCK_HASH) -> dict[str, Any]:
    return {
        ...
        "tcg": {
            "features": {"Packs": False, "Conversion": False, "Ranked": False},
...
class ReleaseSafetyTests(unittest.TestCase):
        ...
        for group, names in sorted(tool.REQUIRED_ARTIFACTS.items()):
            for name in sorted(names):
                ...
                if (group, name) == ("config", "economic-gates"):
                    write_json(path, economic_gates())
                else:
    ...
    def make_try_runtime(self) -> Path:
        return self.make_executable(
            ...
            if sys.argv[1:] == ["--version"]:
                ...
                print("copied-state on-runtime-upgrade passed")
            """,
        )
    ...
    def make_migration_verifier(self, *, unsafe_check: str | None = None) -> Path:
        ...
        return self.make_executable(
            ...
            value = {{
                "schemaVersion": 1,
                "kind": "nexus-v2-v15-v16-migration-result",
                "releaseId": manifest["releaseId"],
                "sourceCommit": manifest["sourceCommit"],
                ...
                "v2Features": {{"Packs": False, "Conversion": False, "Ranked": False}},
                "checks": {checks!r},
                "counts": {{
                    "legacyCardsBefore": 4,
                    "legacyCardsAfter": 4,
